...
- 不使用事件時間猜測來源裝置，只接受 `IOHIDManager` 回報的實體來源。
...
以可丟棄原型驗證 macOS 是否能穩定辨識內建、USB 與藍牙鍵盤的來源裝置，並確認按鍵轉換、權限與事件抑制限制。
...
5. 建立最小 HID usage 至 Carbon virtual key code 對照，測試一般鍵與左右修飾鍵。
6. 同時操作兩把鍵盤，確認每把鍵盤的修飾鍵狀態相互獨立。
7. 測試藍牙鍵盤休眠、喚醒、斷線、重新連線與 App 重啟後的識別穩定性。
...
- 來源裝置辨識在三種鍵盤類型上均正確。
- 藍牙重新連線後至少能穩定比對到同一裝置或同型號範圍。
- 左右修飾鍵與一般按鍵可轉為現有 `Shortcut` 語意。
- 權限狀態可診斷，拒絕時能安全降級。
...
- 單元測試通過。
- 內建、USB、藍牙鍵盤執行裝置加入與移除 smoke test。
...
|---|---:|---:|---:|
| 裝置列舉與名稱 | 必測 | 必測 | 必測 |
| 裝置專用 toggle | 必測 | 必測 | 必測 |
| push-to-talk down/up | 必測 | 必測 | 必測 |
| hybrid 短按與長按 | 必測 | 必測 | 必測 |
| 斷線與重新連線 | 不適用 | 必測 | 必測 |
| 休眠與喚醒 | 必測 | 選測 | 必測 |
| 權限撤銷後降級 | 必測 | 必測 | 必測 |
