...
狀態：Phase 0 至 Phase 9 實作完成，藍牙連線實機驗收待鍵盤切換連線方式
建立日期：2026-08-28
...
- 已完成版本化綁定模型、既有快速鍵遷移、裝置範圍衝突檢查與執行期路由。
- 已完成單一 `IOHIDManager` 監聽、短時間唯一事件配對、裝置拔除狀態清理與權限不足時的保守降級。
- 已完成設定介面的鍵盤選擇、裝置專用錄製、未連線狀態、同型號警告、輸入監控導引、繁體中文與診斷資訊。
- 完整 `xcodebuild test` 已通過，包含單元測試與 UI 測試。
- 2026-08-31 新增藍牙鍵盤的本次連線按鍵來源驗證，驗證成功後可建立或修改綁定。
- 藍牙鍵盤的休眠、喚醒與重新連線仍待依實際硬體完成相容性測試。
- 內建與 USB 鍵盤的 toggle、push-to-talk、hybrid、拔除重連與權限撤銷仍需完成正式 App 實機驗收，因此目前不宣告整項功能已完成發布。

## Phase 9：藍牙驗證實機診斷

### 目標

確認輸入監控已允許但 Wa-Gong 未建立 HID 使用者端時，監聽租約在哪一個畫面生命週期被釋放。

### 成功標準

- 驗證畫面顯示期間，Wa-Gong 持續存在於 `IOHIDLibUserClient` 清單。
- 按下測試字母鍵後，驗證狀態由等待改為完成。
- 最終版本移除所有 `[DEBUG-kbdlease]` 暫時診斷紀錄。

### 測試

- 以 `ioreg -r -c IOHIDLibUserClient -l -w 0` 檢查 Wa-Gong PID。
- 執行既有鍵盤監聽、來源配對與驗證模型單元測試。
- 使用實際藍牙鍵盤完成選擇與按鍵驗證。
...
- 原先的 `IOHIDManager` 只啟用非同步事件派送，未先呼叫 `IOHIDManagerOpen`，因此能列出裝置，卻收不到按鍵輸入。現已在啟用前開啟 manager，開啟失敗時保守停用監聽。
- 實機紀錄確認，使用者選擇「木坨坨 80」藍牙來源後，測試按鍵實際由 `Tree80` 的 USB 來源送出。來源不一致時拒絕通過符合規格。
- 來源不一致時，介面現在會顯示實際偵測到的鍵盤與連線方式，並提供重試，不再持續等待到逾時。
- 已移除實機診斷期間加入的暫時按鍵來源紀錄。待鍵盤切換至藍牙模式後，完成最後一次藍牙實機驗收。
...
- 藍牙配對與連線管理不在範圍內；設定綁定時只驗證目前連線的事件來源。
...
以可丟棄原型驗證 macOS 是否能穩定辨識內建與 USB 鍵盤的來源裝置，並確認按鍵配對、權限與事件抑制限制。藍牙另以本次連線按鍵驗證與實際硬體相容性測試補齊。
...
7. 將藍牙鍵盤休眠、喚醒、斷線、重新連線與 App 重啟列為實際硬體相容性測試。
...
- 權限狀態可診斷，拒絕時能安全降級。
- 藍牙未完成實際硬體測試前，文件需揭露休眠、喚醒與重新連線的相容性限制。
...
- 實機手動核對內建、USB、藍牙與雙鍵盤同時輸入；藍牙另驗證休眠、喚醒與重新連線。
...
- 單元測試通過。
- 內建、USB 與藍牙鍵盤執行裝置加入與移除 smoke test；藍牙另驗證本次連線按鍵來源。
...
- 內建、USB 與未連線狀態均有文字標示；藍牙鍵盤標示「需按鍵驗證」，驗證成功後才開放錄製。
...
- 內建與 USB 鍵盤完成實機驗證。
- 藍牙建立或修改綁定前，必須完成本次連線的按鍵來源驗證。
- 藍牙休眠、喚醒與重新連線仍待依實際硬體完成相容性測試。
- 權限拒絕、撤銷與重新授權流程通過。
